...
All 6 test cases were run using the WaveEye AXI4-Lite Protocol Analyzer (Automated Mode, AXI4-Lite RCA).
...
| [`axil_ram`](alex_forencich_axil_ram/terminal_output.txt) | `axil_ram.v` | 56 | `AXI4L_WRITE_RESPONSE_MISSING` | Always-block NBA override on BVALID — asserting assignment overwritten; 4 violation types detected | 1.5 s |
| [`axil_dp_ram`](alex_forencich_axil_dp_ram/terminal_output.txt) | `axil_dp_ram.v` | 100 | `AXI4L_BVALID_PERSISTENCE` | Always-block NBA override on BVALID — asserting assignment overwritten; 4 violation types detected | 24.2 s |
