...
_SV_KEYWORDS = frozenset([
    'module', 'endmodule', 'always', 'assign', 'if', 'else', 'begin', 'end',
    'case', 'casez', 'casex', 'endcase', 'for', 'while', 'initial', 'input',
    'output', 'inout', 'wire', 'reg', 'logic', 'bit', 'parameter', 'localparam',
    'generate', 'endgenerate', 'function', 'endfunction', 'task', 'endtask',
    'integer', 'real', 'time', 'posedge', 'negedge', 'signed', 'unsigned',
])
...
def detect_instantiations(rtl_lines):
    ...
    while i < n:
        ...
        # Handle split-line style: "module_name inst_name" on this line, "(" on next line
        m_split = re.match(r"\s*(\w+)\s+([A-Za-z_]\w*)\s*$", line)
        if m_split:
            submodule, inst_name = m_split.groups()
            if submodule not in _SV_KEYWORDS and inst_name not in _SV_KEYWORDS:
                j = i + 1
                while j < n and not strip_comments(rtl_lines[j]).strip():
                    j += 1
                if j < n and re.match(r"\s*\(", strip_comments(rtl_lines[j])):
                    full_block, end_i = accumulate_until_close(rtl_lines, j)
                    m_pos = re.search(r"\(", full_block)
                    port_block = full_block[m_pos.end():] if m_pos else full_block
                    connections = {p: s for p, s in extract_port_pairs(port_block)}
                    instantiations.append({"submodule": submodule, "instance": inst_name, "connections": connections})
                    i = end_i + 1
