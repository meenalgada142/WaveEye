...
def main():
    ...
    # STEP 2: Classify Signals (all RTL files combined in one pass)
    print("\n==========================")
    print("STEP 2: Classify Signals (All RTL Files Combined)")
    print("==========================")

    classify_signals_py = os.path.join(RTL_DIR, "classify_signals.py")

    # Pass the RTL directory so classify_signals reads all .sv/.v files together
    all_out_prefix = os.path.join(OUTPUT_DIR, "all_signals")
    print(f"\n-> Classifying all RTL files in: {USER_INPUT}")
    run_step(classify_signals_py, [USER_INPUT, all_out_prefix])

    all_signals_json = f"{all_out_prefix}_signals.json"
    all_signals_csv  = f"{all_out_prefix}_signals.csv"

    if os.path.exists(all_signals_json):
        metadata_files = [("all_signals", all_signals_json)]
        print(f"[OK] Combined signals JSON: {all_signals_json}")
    elif os.path.exists(all_signals_csv):
        metadata_files = [("all_signals", all_signals_csv)]
        print(f"[OK] Combined signals CSV: {all_signals_csv}")
    else:
        print("[WARN] Combined classify failed — falling back to per-file classification")
        metadata_files = []
        for rtl_file in rtl_files:
